...
# Каждый запуск пишет в отдельный лог-файл с timestamp в имени.
# Предупреждения и ошибки дублируются в отдельный err-лог того же запуска.
LOG_TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
LOG_DIR="$(dirname "$LOG_FILE")"
LOG_BASE="$(basename "$LOG_FILE" .log)"
LOG_FILE="${LOG_DIR}/${LOG_BASE}_${LOG_TIMESTAMP}.log"
ERR_FILE="${LOG_DIR}/${LOG_BASE}_${LOG_TIMESTAMP}.err.log"

mkdir -p "$LOG_DIR"

# log_file    — только в основной лог (детали, отладка)
# log_step    — в основной лог + stdout
# log_success — в основной лог + stdout (зелёный)
# log_warning — в основной лог + err-лог + stdout (жёлтый)
# log_error   — в основной лог + err-лог + stdout (красный), завершает скрипт
...
log_warning() {
    local msg="$(date '+%Y-%m-%d %H:%M:%S.%3N') - WARNING: $1"
    echo -e "\033[1;33m${msg}\033[0m" | tee -a "$LOG_FILE"
    echo "$msg" >> "$ERR_FILE"
}
log_error() {
    local msg="$(date '+%Y-%m-%d %H:%M:%S.%3N') - ERROR: $1"
...
export_ddl() {
    ...
    local exp_tables=0 exp_dist=0 exp_dicts=0 exp_views=0 exp_mvs=0

    for db in $databases; do
        log_file "  Экспорт базы: $db"
        ...
        while read -r tbl; do
            ...
            ch_old "SHOW CREATE TABLE \`$db\`.\`$tbl\`" \
                > "$BACKUP_DIR/ddl/tables/${db}___$(encode_name "$tbl").sql"
            exp_tables=$(( exp_tables + 1 ))
        done < <(ch_old "SELECT name FROM system.tables
                WHERE database = '$db'
        ...
        while read -r tbl; do
            [ -z "$tbl" ] && continue
            ch_old "SHOW CREATE TABLE \`$db\`.\`$tbl\`" \
                > "$BACKUP_DIR/ddl/distributed/${db}___$(encode_name "$tbl").sql"
            exp_dist=$(( exp_dist + 1 ))
        done < <(ch_old "SELECT name FROM system.tables
        ...
        while read -r dict; do
            ...
            ch_old "SHOW CREATE DICTIONARY \`$db\`.\`$dict\`" \
                > "$BACKUP_DIR/ddl/dictionaries/${db}___$(encode_name "$dict").sql"
            exp_dicts=$(( exp_dicts + 1 ))
        done < <(ch_old "SELECT name FROM system.dictionaries
                WHERE database = '$db'")
        ...
        while read -r view; do
            ...
            ch_old "SHOW CREATE TABLE \`$db\`.\`$view\`" \
                > "$BACKUP_DIR/ddl/views/${db}___$(encode_name "$view").sql"
            exp_views=$(( exp_views + 1 ))
        done < <(ch_old "SELECT name FROM system.tables
        ...
        while read -r mv; do
            ...
            ch_old "SHOW CREATE TABLE \`$db\`.\`$mv\`" \
                > "$BACKUP_DIR/ddl/matviews/${db}___$(encode_name "$mv").sql"
            exp_mvs=$(( exp_mvs + 1 ))
        done < <(ch_old "SELECT name FROM system.tables
                WHERE database = '$db' AND engine = 'MaterializedView'")
    done

    log_success "Экспорт DDL завершён: таблиц=${exp_tables} distributed=${exp_dist} словарей=${exp_dicts} view=${exp_views} matview=${exp_mvs}"
}
...
apply_file() {
    ...
    local err_out
    # Захватываем stderr сразу — запрос выполняется ровно один раз.
    # stdout (вывод вида "host port 0 3 0" от ON CLUSTER) подавляем.
    err_out=$(ch_new "$(cat "$file")" "$fatal" 2>&1 >/dev/null)
    local rc=$?

    if [ $rc -eq 0 ]; then
        log_file "  ✓ $label"
    else
        [ -n "$err_out" ] && echo "$err_out" >> "$LOG_FILE"
...
parse_filename() {
    ...
    local -n _name="$3"
    _db="${fname%%___*}"
    _name=$(decode_name "${fname#*___}")
}

# Заменяет / на %2F — для безопасного использования имени сущности в имени файла
encode_name() {
    echo "${1//\//%2F}"
}

# Восстанавливает / из %2F — при чтении имени сущности из имени файла
decode_name() {
    echo "${1//%2F/\/}"
}
...
apply_ddl() {
    ...
    local ddl_dbs=0 ddl_tables=0 ddl_dist=0 ddl_dicts=0 ddl_views=0
    ...
    log_step "  Базы данных: ${ddl_dbs}"

    # 2.2 Локальные таблицы
    for f in "$BACKUP_DIR/ddl/tables/"*.sql; do
        [ -f "$f" ] || continue
        local fname db tname
        fname=$(basename "$f" .sql); parse_filename "$fname" db tname
        [ -n "$TARGET_DB" ] && [ "$db" != "$TARGET_DB" ] && continue
        prepare_ddl_file "$f" "table"
        apply_file "$f" "TABLE $db.$tname" true
        ddl_tables=$(( ddl_tables + 1 ))
    done
    log_step "  Локальные таблицы: ${ddl_tables}"

    # 2.3 Distributed таблицы
    ...
    log_step "  Distributed таблицы: ${ddl_dist}"

    # 2.4 Словари
    for f in "$BACKUP_DIR/ddl/dictionaries/"*.sql; do
        [ -f "$f" ] || continue
        local fname db dname
        fname=$(basename "$f" .sql); parse_filename "$fname" db dname
        [ -n "$TARGET_DB" ] && [ "$db" != "$TARGET_DB" ] && continue
        prepare_ddl_file "$f" "dictionary"
        apply_file "$f" "DICTIONARY $db.$dname" false
        ddl_dicts=$(( ddl_dicts + 1 ))
    done
    log_step "  Словари: ${ddl_dicts}"

    # 2.5 View
    for f in "$BACKUP_DIR/ddl/views/"*.sql; do
        [ -f "$f" ] || continue
        local fname db vname
        fname=$(basename "$f" .sql); parse_filename "$fname" db vname
        [ -n "$TARGET_DB" ] && [ "$db" != "$TARGET_DB" ] && continue
        prepare_ddl_file "$f" "view"
        apply_file "$f" "VIEW $db.$vname" false
        ddl_views=$(( ddl_views + 1 ))
    done
    log_step "  View: ${ddl_views}"

    log_success "Применение DDL завершено: баз=${ddl_dbs} таблиц=${ddl_tables} distributed=${ddl_dist} словарей=${ddl_dicts} view=${ddl_views}"
}
...
migrate_data() {
    ...
    local mig_ok=0 mig_skip=0 mig_err=0
    ...
    for db in $databases; do
        ...
        local mig_total mig_done=0
        mig_total=$(echo "$tables" | grep -c '[^	]' || echo 0)

        while IFS=$'	' read -r table engine; do
            [ -z "$table" ] && continue
            mig_done=$(( mig_done + 1 ))
            log_step "  [$mig_done/$mig_total] $db.$table [$engine]"
...
migrate_table() {
    ...
    if [ "$old_count" = "0" ]; then
        log_file "  \`$db\`.\`$table\` [$engine] — пусто на старом, пропускаем"
        return
    fi

    # Проверяем сколько строк уже на новом кластере —
    # защита от дублей при повторном запуске скрипта.
    local new_count
    ...
    if [ "$new_count" -ge "$old_count" ]; then
        log_step "  ⏭ \`$db\`.\`$table\` — уже перенесено ($new_count строк), пропускаем"
        return
    fi

    if [ "$new_count" -gt 0 ]; then
        # Частичная вставка от предыдущего запуска — очищаем перед повтором.
        log_warning "  ⚠ \`$db\`.\`$table\` — частично заполнено (new=$new_count old=$old_count), очищаем..."
        if [ "$engine" = "Distributed" ]; then
            ch_new "TRUNCATE TABLE \`$db\`.\`$table\` ON CLUSTER ${CLUSTER_NAME}" ||                 { log_warning "  TRUNCATE не удался, пропускаем таблицу"; return; }
        else
            ch_new "TRUNCATE TABLE \`$db\`.\`$table\`" ||                 { log_warning "  TRUNCATE не удался, пропускаем таблицу"; return; }
        fi
    fi

    log_step "  \`$db\`.\`$table\` [$engine] — $old_count строк, переносим..."
...
# ── Шаг 4: Materialized View ─────────────────────────────────────────────────
# Создаём строго после переноса данных — иначе INSERT триггернёт MV
...
apply_matviews() {
    ...
    log_success "Materialized View созданы: ${mv_count}"
}
...
main() {
    ...
    log_success "  Лог:     $LOG_FILE"
    log_success "  Err-лог: $ERR_FILE"
    log_success "=================================================="
}
