#!/bin/bash
# =============================================================================
# seed_data.sh  –  Populate the OLD cluster (TLS) with test data for script2.
#
# Identical schema to script1's seed so results are directly comparable.
# All connections use --secure --no-verify (self-signed CA).
# =============================================================================
...
q1() {
    clickhouse-client \
        --host="$OLD_SHARD1" --port=9440 \
        --secure --no-verify \
        --multiquery -q "$1"
}
q2() {
    clickhouse-client \
        --host="$OLD_SHARD2" --port=9440 \
        --secure --no-verify \
        --multiquery -q "$1"
}
qboth() { q1 "$1"; q2 "$1"; }
...
echo "=== analytics.events_local (MergeTree) ==="
qboth "
...
CREATE TABLE IF NOT EXISTS analytics.events_dist (
    ...
    duration_ms  UInt32,
    country      LowCardinality(String)
) ENGINE = Distributed('main', 'analytics', 'events_local', user_id);
"
...
CREATE TABLE IF NOT EXISTS analytics.metrics_dist (
    ...
    error_count   UInt64,
    total_ms      UInt64
) ENGINE = Distributed('main', 'analytics', 'metrics_local', cityHash64(service, endpoint));
"
...
echo "=== analytics.page_views (ReplacingMergeTree with version col) ==="
qboth "
...
echo "=== analytics.audit_log (MergeTree) ==="
qboth "
...
echo "=== inventory.stock_movements (CollapsingMergeTree) ==="
qboth "
...
CREATE TABLE IF NOT EXISTS inventory.products_dist (
    ...
    price       Decimal(10,2),
    created_at  DateTime
) ENGINE = Distributed('main', 'inventory', 'products', product_id);
"
...
echo ""
echo "=== Row count summary on OLD cluster (TLS) ==="
for db_table in \
...
do
    c1=$(clickhouse-client --host="$OLD_SHARD1" --port=9440 --secure --no-verify \
         -q "SELECT count() FROM $db_table" 2>/dev/null || echo 0)
    c2=$(clickhouse-client --host="$OLD_SHARD2" --port=9440 --secure --no-verify \
         -q "SELECT count() FROM $db_table" 2>/dev/null || echo 0)
